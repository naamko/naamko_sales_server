...
router.get("/export/excel", async (req: Request, res: Response) => {
    try {
        ...
        const filename = `location-reports-${new Date().toISOString().split("T")[0]
            }.xlsx`;
...
interface LocationCoordinateRow {
    location_id: string;
...
// Public: Get all coordinates for map display
router.get("/coordinates", async (req: Request, res: Response) => {
    try {
        console.log("🗺️ Fetching coordinates for map display...");
    ...
    `;
        ...
        const coordinates = result.rows.map((row: LocationCoordinateRow) => ({
        ...
        res.status(200).json({
            ...
            message: `✅ Retrieved ${coordinates.length} location coordinates`
        });
